...
from app.domain.ports.preprocessor import Preprocessor


class TechnicalIndicatorEnricherPreProcessor(Preprocessor):
    @staticmethod
    ...
    def execute(cls, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["bb_width"] = (bb_upper - bb_lower) / bb_sma  # Normalized width
        raw_band_width = bb_upper - bb_lower
        df["bb_position"] = np.where(
            df["bb_width"] > 1e-5,  # Check width
            (df["close"] - bb_lower) / raw_band_width,  # Use raw band width
            0.5,  # Middle position when bands are too narrow
        )
        ...
        df["is_weekend"] = (df.index.dayofweek >= 5).astype(int)  # type: ignore

...
from app.domain.ports.target_builder import TargetBuilder
...
class TPSLTargetBuilder(TargetBuilder):
    def __init__(
        self,
        exchange: Exchange,
    ...
    def execute(self, df: pd.DataFrame) -> list[int]:
        ...
        for i in range(len(df)):
            ...
            if up_first_idx is not None and down_first_idx is not None:
                ...
                up_first_pos = cast(int, future_prices.index.get_loc(up_first_idx))
                down_first_pos = cast(int, future_prices.index.get_loc(down_first_idx))
